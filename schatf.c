...
char encrypt(char c) {
	...
	while(fgets(buffer,512,table) != NULL){
		sscanf("%[^';'];%s",index,encrypted);
		if(index == c) {
			printf(ANSI_COLOR_GREEN "encrypt: Successful");
			printf(ANSI_COLOR_WHITE "\n" );
			fclose(table);
			return encrypted;
		}
	}
	printf(ANSI_COLOR_RED "encrypt: index not found");
	printf(ANSI_COLOR_WHITE "\n");
	fclose(table);
	return "";
}
 ...
 * --------------------
 * Send message to user trying to connect to us
 *
 * returns: 0 if successful
 *          -1 if error ocurred or authentication not correct
 */
int authenticate() {
	int randNum;
	char randChar,encrypted;
	...
	/* Send and receive from person trying to connect to us */
	/* ... */
	...
	printf(ANSI_COLOR_RED "authenticate: User authentication not successful");
	printf(ANSI_COLOR_WHITE "\n");
	return -1;
}
